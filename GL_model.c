...
void jarzmo(void)
{
	...
	glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
	{
		...
		glBegin(GL_TRIANGLE_FAN);
		glColor3d(0.1, 0.1, 0.1);
		glVertex3d(-31, -43, 27.5);
		for (alpha = 0 * PI / 8.0; alpha >= -7 * PI / 8.0; alpha -= PI / 8.0)
		{
			z = 24.5 * cos(alpha) + 27.5;
			x = 24.5 * sin(alpha) - 31;
			glVertex3d(x, -43, z);
		}
		glEnd();
		//chyba trzeba bêdzie zrobiæ œcianê œrodka jarzma
		glBegin(GL_QUAD_STRIP);
		glColor3d(0.4, 0.4, 0.4);
		for (alpha = 4 * PI / 8.0; alpha <= 11 * PI / 8.; alpha += PI / 8.0)
		{
			z = 24.5 * sin(alpha) + 27.5;
			x = 24.5 * cos(alpha) - 31;
			glVertex3d(x, -43, z);
			glVertex3d(x, 43, z);
		}
...
void spirala(double r, double h)
{
	double x = 0, y = 0, z = 0, alpha, PI = 3.14;
...
void schodki(double r, double h)
{
	double x1, x2, y1, y2, z = 0, alpha1, alpha2, PI = 3.14;
	glBegin(GL_TRIANGLES);
	glColor3d(0, 0, 0);
	for (alpha1 = 0; alpha1 <= h * PI; alpha1 += PI / 8.0)
	{
		alpha2 = alpha1 + PI / 8.0;
		x1 = r * sin(alpha1);
		y1 = r * cos(alpha1);
		x2 = r * sin(alpha2);
		y2 = r * cos(alpha2);
